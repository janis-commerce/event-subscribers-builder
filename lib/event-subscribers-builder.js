...
class EventSubscribersBuilder {
	...
	async _resolveEndpoints(environment, schemas, events) {

		if(!Array.isArray(events))
			throw new EventSubscribersBuilderError('Invalid yml: Should be an array.', EventSubscribersBuilderError.codes.INVALID_YML);
		...
		for(const event of events) {
			...
			for(const listener of event.listeners) {
				...
				try {
					...
					const { url } = await endpointResolver.resolve(listener.namespace, listener.method);
					resolvedEvent.listeners = [...resolvedEvent.listeners, url];
	...
	async execute(environment = this.environment, input = this.inputPath, output = this.outputPath, schemas = this.schemasPath) {
		...
		try {
			...
			// Prevents an error when the source yml is empty
			if(eventsYml !== null)
